...
namespace Wexflow.Tasks.DockerRestartContainer
{
    public class DockerRestartContainer : Task
    {
        ...
        public DockerRestartContainer(XElement xe, Workflow wf) : base(xe, wf)
        {
        ...
        public override TaskStatus Run()
        {
            ...
            try
            {
                ...
                using (HttpClient client = new HttpClient(handler))
                {
                    ...
                    HttpResponseMessage response = client.PostAsync("/containers/" + ContainerId + "/restart", null).Result;

                    if (response.IsSuccessStatusCode)
                    {
                        Info("The container was restarted.");
                        return new TaskStatus(Status.Success, true);
                    }
                    else
                    {
                        ErrorFormat("An error occured while restarting the container. Status code: {0}", response.StatusCode);
            ...
            {
                ErrorFormat("An error occured while restarting the container. Error: {0}", e.Message);
                return new TaskStatus(Status.Error, false);
            }
        }
