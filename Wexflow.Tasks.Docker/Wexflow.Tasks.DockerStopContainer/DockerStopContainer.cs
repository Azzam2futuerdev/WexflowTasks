...
namespace Wexflow.Tasks.DockerStopContainer
{
    public class DockerStopContainer : Task
    {
        ...
        public DockerStopContainer(XElement xe, Workflow wf) : base(xe, wf)
        {
        ...
        public override TaskStatus Run()
        {
            ...
            try
            {
                ...
                using (HttpClient client = new HttpClient(handler))
                {
                    ...
                    HttpResponseMessage response = client.PostAsync("/containers/" + ContainerId + "/stop", null).Result;

                    if (response.IsSuccessStatusCode)
                    {
                        Info("The container was stopped.");
                        return new TaskStatus(Status.Success,true);
                    }
                    else
                    {
                        ErrorFormat("An error occured while stopping the container. Status code: {0}", response.StatusCode);
            ...
            {
                ErrorFormat("An error occured while stopping the container. Error: {0}", e.Message);
                return new TaskStatus(Status.Error, false);
            }
        }
